...
public class PlayerVSPlayer extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (player_Names[0].length()==0){
            binding.p1name.setText(R.string.player_1);
        }else {
            binding.p1name.setText(player_Names[0]);
        }
        if (player_Names[1].length()==0){
            binding.p2name.setText(R.string.player_2);
        }else {
            binding.p2name.setText(player_Names[1]);
        }
        String[] final_Names = new String[]{binding.p1name.getText().toString(), binding.p2name.getText().toString()};
        ...
        binding.player2Dice.setOnClickListener(v -> {
            if (!handlerActive) dice2Roll(0);
        });
